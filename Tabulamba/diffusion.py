...
class GaussianDiffusion(torch.nn.Module):
    ...
    def compute_loss_with_class_stats(self, x, y):
        """计算损失，同时返回按类别的损失"""
        b = x.shape[0]
        device = x.device
        t, pt = self.sample_time(b, device, 'uniform')
        ...
        per_sample_loss = self._gaussian_loss_per_sample(model_out, x, x_t, t, noise)
        ...
        for class_id in unique_classes:
            mask = (y == class_id)
            if mask.sum() > 0:
                loss_per_class[class_id.item()] = per_sample_loss[mask].mean().item()
            else:
                loss_per_class[class_id.item()] = None

        return total_loss, loss_per_class
    ...
    def _gaussian_loss_per_sample(self, model_out, x_start, x_t, t, noise, model_kwargs=None):
        ...
        if self.gaussian_loss_type == 'mse':
            loss_per_sample = ((noise - model_out) ** 2).mean(dim=tuple(range(1, len(x_start.shape))))
        elif self.gaussian_loss_type == 'kl':
            loss_per_sample = self._vb_terms_bpd_per_sample(
                model_output=model_out,
                x_start=x_start,
                x_t=x_t,
                t=t,
    ...
    @torch.no_grad()
    def sample(self, num_samples, cond_fn, clip_denoised=True):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.input_dim), device=device)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            z_norm = self.gaussian_p_sample(model_out, z_norm, t, cond_fn, clip_denoised=clip_denoised)['sample']
      
        print()
        sample = z_norm
        return sample
    
    def batch_sample(self, num_samples,cond_fn, batch_size, clip_denoised=True):
        ...
        for batch_i in range(no_batches):
            ...
            sample = sample_fn(batch_end-batch_start, cond_fn, clip_denoised)
