...
class GaussianDiffusion(torch.nn.Module):
    ...
    @torch.no_grad()
    def sample(self, num_samples, cond_fn, clip_denoised=False):
        b = num_samples
        device = self.log_alpha.device
        #self.input_dim 是每个样本的特征维度
        #使用标准正态分布初始化噪声 z_norm，这是扩散模型生成样本的起点
        z_norm = torch.randn((b, self.input_dim), device=device)

        for i in reversed(range(0, self.num_timesteps)):
            print(f'Sample timestep {i:4d}', end='\r')
            #为当前批次中的所有样本创建相同的时间步张量
            t = torch.full((b,), i, device=device, dtype=torch.long)
            ...
            #从gaussian_p_sample返回的字典中提取名为 'sample' 的键对应的值
