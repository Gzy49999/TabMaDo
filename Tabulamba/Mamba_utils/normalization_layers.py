...
class RMSNorm(nn.Module):
    ...
    def __init__(self, d_model, eps: float = 1e-5,device = "cuda:0"):
        super().__init__()
        self.eps = eps
        self.d_model = d_model
        self.weight = nn.Parameter(torch.ones(1,self.d_model))
        self.device = device

    def forward(self, x):
        #对最后一维（d_model）求平方后取平均 → 得到每 token 的 均方值，形状 (B, L, 1)
        # rsqrt = 平方根再取倒数，即1 / sqrt(...)。
        # eps防止除零
        #先把 x 归一化到单位 RMS，再乘以可学习权重，完成 缩放
