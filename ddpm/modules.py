...
#Sigmoid Linear Unit (SiLU) 激活函数
class SiLU(nn.Module):
    def forward(self, x):
        return x * torch.sigmoid(x)

#生成时间步的正弦嵌入向量，用于扩散模型中的时间步编码,在扩散模型中，时间步的嵌入向量用于将时间信息注入到模型中
...
class MLPClassifier(nn.Module):
    def __init__(self, d_in, d_layers, num_classes, dropout, dim_t = 128, t_in=False):
        ...
        #一个线性层，将输入特征从维度 d_in 投影到维度 dim_t
    ...
    def forward(self, x, t=None):
        ...
        x = self.mlp(x)
        x = self.head(x)
        #如果类别数大于 2，使用 Softmax 函数将输出转换为概率分布
        #输出是一个概率分布，表示输入数据属于每个类别的概率
        if self.num_classes > 2:
            return torch.softmax(x, dim=1)
        #否则，使用 Sigmoid 函数将输出转换为概率值（适用于二分类任务）
        #输出是一个概率值，表示输入数据属于label1的概率
        else:
