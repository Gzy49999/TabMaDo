...
def sample_data_control(diff_model, data_wrapper, condition, scorer, weight):
	...
	while len(sample_index) > 0:
		...
		sample = diff_model.sample_all(len(cond_input), batch_size=200000, clip_denoised=False, control_tools=control_tools, control_t=200)
		#sample = diff_model.sample(len(cond_input), control_tools=control_tools, clip_denoised=True, control_t=control_t)
		sample = sample.cpu().numpy()
		sample = data_wrapper.ReverseToOrdi(sample)
